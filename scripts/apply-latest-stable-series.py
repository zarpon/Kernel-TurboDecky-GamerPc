#!/usr/bin/env python3
"""Follow the resolved Linux series for non-scheduler build rewrites.

BORE is finalized from the build's dynamic patch lock after every source
resolver run. Series-specific compatibility rewrites below are intentionally
optional: earlier upstream-only rewriters may already have removed the legacy
anchors. Functional injections still fail closed when their required anchors
are absent.
"""
...
def replace_legacy_if_present(text: str, old: str, new: str) -> str:
    """Upgrade one legacy identity without requiring it to still exist.

    The upstream-only resolver can replace the whole candidate block before
    this stage runs. In that state neither the old nor the historical dynamic
    spelling needs to be present, and that is a valid no-op rather than a
...
def patch_core(path: Path) -> None:
    ...
    for old, new in legacy_series_replacements.items():
        source = replace_legacy_if_present(source, old, new)
    ...
    for old, new in behavioral_replacements.items():
        if old in source:
