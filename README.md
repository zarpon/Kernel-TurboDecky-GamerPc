...
`mitigations=off` e `nowatchdog`; isso pode
...
- **BORE scheduler** — [firelzrd/bore-scheduler](https://github.com/firelzrd/bore-scheduler/tree/main/patches/testing): ajuste de prioridade CFS/EEVDF orientado pelo tempo de burst. A cada build, o resolvedor seleciona e bloqueia por commit, caminho, SHA-256 e tamanho a versão mais nova com série Linux exata. O patch é aplicado apenas após `patch --dry-run`, sem fuzz. A correção `sched_ext` também é bloqueada; seu port Linux 7.1 é materializado do corpo upstream atual de `reweight_task()` e do contexto revisado, com hash, tamanho e SHA da fonte pai registrados no lock. Uma mudança incompatível de estrutura falha antes da compilação ou publicação.
