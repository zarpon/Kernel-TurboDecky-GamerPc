...
- **Memory:** Marie LRU reduces aggressive reclaim and thrashing; ZRAM-IR uses
  LZ4KDR as its primary compressor and ZSTD for recompression to retain more
  useful pages under memory pressure.
...
- **BORE scheduler 6.8.0-rc1** — [firelzrd/bore-scheduler](https://github.com/firelzrd/bore-scheduler/tree/main/patches/testing): burst-time-oriented CFS/EEVDF priority adjustment. The resolver checks the official `testing` and `stable` trees for Linux 7.1 and selects the newest version; the reviewed Linux 7.1.4 port is stored at `patches/bore/7.1.4-bore-6.8.0-rc1.patch`. The official patch SHA-256 must match the SHA reviewed for the port, so an upstream update stops the build until the new port is validated. The lock also tracks `0002-sched-ext-coexistence-fix.patch`; its local port is applied immediately after BORE, restores `reweight_task()`, and adds the prototype required for strict compilation.
...
- **ZRAM-IR + LZ4KDR** — [firelzrd/zram-ir](https://github.com/firelzrd/zram-ir):
  LZ4KDR as the primary compressor and ZSTD as priority `1` recompressor. The
  LZ4KDR backend is ported to the current stable series and exposed to zswap;
  select it at boot with `zswap.compressor=lz4kdr`. The
