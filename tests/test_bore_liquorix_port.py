...
class BoreLinuxPortTests(unittest.TestCase):
    def test_build_defers_bore_to_the_exact_dynamic_lock(self) -> None:
        ...
        implementation = FINALIZER_BASE.read_text(encoding="utf-8")
        self.assertIn('with_name("finalize-bore-stable-port-base.py")', finalizer)
        self.assertIn("load_locked_bore", finalizer)
        self.assertIn('BORE_PATCH="$RESOLVED_PATCH_ROOT/{output}"', implementation)
        self.assertNotIn("6.8.0-rc1", implementation)
